...
class Body2d:
    ...
    def draw(self, image, only_segments=False):
        ...
        color = (int(body_colors[self.id][0]), int(
            body_colors[self.id][1]), int(body_colors[self.id][2]))
    ...
    def create(body_handle, calibration, bodyIdx, dest_camera):
        ...
        for jointID, joint in enumerate(body_handle.skeleton.joints):
            skeleton2d_handle.joints2D[jointID].position, _ = calibration.convert_3d_to_2d(
