...
class Transformation:
    ...
    def __init__(self, calibration):
        ...
        self.color_resolution = Resolution(calibration.handle(
        ).color_camera_calibration.resolution_width, calibration.handle().color_camera_calibration.resolution_height)
        self.depth_resolution = Resolution(calibration.handle(
        ).depth_camera_calibration.resolution_width, calibration.handle().depth_camera_calibration.resolution_height)
    ...
    def depth_image_to_color_camera_custom(self, depth_image, custom_image, interpolation=_k4a.K4A_TRANSFORMATION_INTERPOLATION_TYPE_LINEAR):
        ...
        _k4a.k4a_transformation_depth_image_to_color_camera_custom(self._handle, depth_image.handle(), custom_image.handle(
        ), transformed_depth_image.handle(), transformed_custom_image.handle(), interpolation, invalid_custom_value)
    ...
    def color_image_to_depth_camera(self, depth_image, color_image):
        ...
        _k4a.k4a_transformation_color_image_to_depth_camera(
            self._handle, depth_image.handle(), color_image.handle(), transformed_color_image.handle())
